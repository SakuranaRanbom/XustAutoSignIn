...
//        text();
        String val = "2020-06-07 19:07:26";
...
//    }
    ...
    public static boolean start(User user) throws IOException {
        ...
        int hh = Integer.parseInt(lastTime.substring(11, 13));

        // 提前check一下 如果已经打过卡 而且为在校的 则直接return;
        if( (user.getIn().equals("1") && check(user, cookie) && lastTime.substring(0, 10).equals(sdf.format(new Date()))) || ((user.getIn().equals("2") && check(user, cookie) && lastTime.substring(0, 10).equals(sdf.format(new Date()))))){
            template.update("insert into logs values(?, ?, ?)",  "学号:" + user.getGh() + " 姓名:" + user.getName(), "晚上打卡成功,时间为" + lastTime, new Date());
            return true;
        }
...
//        tmpJsonObject.remove("id");
        ...
        bufferedWriter.write("学号: " + tmpJsonObject.getString("gh") + " 姓名: " + tmpJsonObject.getString("xm") + " 在" + sdf.format(new Date()) + "进行了SingnIn操作");
